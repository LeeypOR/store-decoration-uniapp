<template>
    <view class="kill">
        <view class="header">
            ...
            <image class="kill-image" src="/static/image/top_xianshizhekou@2x.png"></image>
            <view class="bottom">
            ...
            >
                ...
                <view
                    ...
                    :class="[
                                ...
                                {'disable':data[currentIndex].activityStatu==1||data[currentIndex].activityStatu==3}
                            ]"
                    :style="{'margin-bottom':dataInfo.imagePadding*pxToRpxScale+'rpx'}"
                    v-for="(item,index) in data[currentIndex].discountGoodsResponses"
                    :key="index"
                ...
                >
                    <view style="position:relative">
                      ...
                      <view :style="[collageBg]" v-if="dataInfo.listStyle == '1'&& dataInfo.countdownShow "
                            class="mark-acitve-item">
                    ...
                    <view class="info-wrapper" :class="{detail:dataInfo.listStyle==1}" >
                        <view class="info-head" v-if="dataInfo.shopName">
                            <view class="sale" >减{{(item.salePrice-item.discountPrice)/100}}元</view>
                        ...
                        <view class="price-wrapper" :class="{mt10:dataInfo.listStyle==1}"  v-if="dataInfo.listStyle==1" >
                            <text v-if="dataInfo.seckillPrices">折扣价¥<text class="price-number">{{item.discountPrice/100}}</text>起</text>
                        ...
                        <view class="progress" v-if="dataInfo.listStyle==0">
                            <view class="progress-wrapper" v-if="dataInfo.progressBar">
                                <view class="progress-bar" :style="{'width':`${item.totalRemainStock*100/item.totalDiscountStock}%`}"></view>
                            </view>
...
export default {
    ...
    computed:{
        ...
        currentEndTime() {
          ...
          if(this.data[this.currentIndex]?.activityStatu == 2){
...
　　　　
    ...
    methods:{
        ...
        async getDiscountList() {
            let res = await this.$http.getDiscountList(this.list)
        ...
        goToDetail(id,obj){
            uni.setStorageSync('moduleDate', {jumpPageName:obj?.goodsName || '商品详情' ,moduleName:'限时折扣'});
            this.goTo(`/pagesA/shop/goodsDetail?id=${id}&activeType=ZK&activeId=${this.data[this.currentIndex].activityId}`)
        }
    },
