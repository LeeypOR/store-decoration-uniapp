...
export function drawSquarePic(ctx, x, y, w, h, r, url) {
  ...
  ctx.beginPath()
  // 绘制左上角圆弧
  ctx.arc(x + r, y + r, r, 1*Math.PI, Math.PI * 1.5)
  // 绘制border-top
  // 画一条线 x终点、y终点
  ctx.lineTo(x + w - r, y)
  ctx.lineTo(x + w, y + r);
  ...
  ctx.lineTo(x, y + h - r);
  // 绘制左下角圆弧
  ctx.arc(x + r, y + h - r, r, Math.PI * 0.5, 1*Math.PI)
  // 绘制border-left
  ctx.lineTo(x, y + r)
  ctx.lineTo(x + r, y)
  // 填充颜色(需要可以自行修改)
...
export function getSystem() {
  ...
  return { w: system.windowWidth, h: system.windowHeight, scale: scale ,statusBarHeight:system.statusBarHeight}
}
...
export function drawTextReturnH(
  ...
  for (let n = 0; n < arrText.length; n++) {
    ...
    ctx.font = uni.upx2px(fontSize) + 'px blod sans-serif' //设置字体大小，注意：百度小程序 用ctx.setFontSize设置字体大小后，计算字体宽度会无效
	...
	if(lineThrought){//划线价格
		ctx.stroke()
		ctx.moveTo(x, y - uni.upx2px(fontSize) / 3);
		ctx.lineTo(x + testWidth, y - uni.upx2px(fontSize) / 3);
