...
export default class Painter {
  ...
  paint(callback) {
    ...
    this._background();
    if (JSON.stringify(this.posterData)!='{}') {
      let priceH = this._drawTextReturnH(
        '¥'+this.posterData.salesPrice.price,
        this.posterData.salesPrice.left.toPx(),
        this.posterData.salesPrice.top.toPx(),
        this.posterData.salesPrice.fontSize,
        'normal',
        this.posterData.salesPrice.color,
      )
      if (this.posterData?.markPrice) {
        this._drawTextReturnH(
          '¥' + this.posterData.markPrice.price,
          priceH.x + priceH.testWidth + this.posterData.markPrice.ml.toPx() ,
          this.posterData.salesPrice.top.toPx(),
  ...
  _drawTextReturnH(
    ...
    let line = ''
      let testWidth
      let maxWidth = getApp().globalData.systemInfo.windowWidth
    for (let n = 0; n < arrText.length; n++) {
      let testLine = line + arrText[n]

      const fontWeights = fontWeight == 'bold' ? 'bold' : 'normal';
      this.ctx.font = `normal ${fontWeights} ${fontSize.toPx()}px sans-serif` //设置字体大小，注意：百度小程序 用ctx.setFontSize设置字体大小后，计算字体宽度会无效
      this.ctx.setFillStyle(color) //设置字体颜色
      let metrics = this.ctx.measureText(testLine) //measureText() 方法返回包含一个对象，该对象包含以像素计的指定字体宽度。
    ...
    if(lineThrought){//划线价格
      this.ctx.stroke()
      this.ctx.moveTo(x, y - fontSize.toPx() / 3);
      this.ctx.lineTo(x + testWidth, y - fontSize.toPx() / 3);
    ...
    this.ctx.draw(true) //本次绘制是否接着上一次绘制。即 reserve 参数为 false，则在本次调用绘制之前 native 层会先清空画布再继续绘制；若 reserve 参数为 true，则保留当前画布上的内容，本次调用 drawCanvas 绘制的内容覆盖在上面，默认 false。
    return {x,y,testWidth}
  }
  _fillAbsText(view) {
    if (!view.text) {
      return;
